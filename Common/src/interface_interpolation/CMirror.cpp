    ...
    for (int iProcessor = 0; iProcessor < nProcessor; ++iProcessor) {
      ...
      for (int iSend = 0; iSend < nSend; ++iSend) {
        ...
        if ((rank == iProcessor) && (rank == jProcessor)) {
          ...
          SU2_MPI::Recv(GlobalIndex[iSend], numCoeff, MPI_LONG, jProcessor, 0, MPI_COMM_WORLD, nullptr);
          SU2_MPI::Recv(DonorIndex[iSend],  numCoeff, MPI_LONG, jProcessor, 0, MPI_COMM_WORLD, nullptr);
          SU2_MPI::Recv(DonorCoeff[iSend], numCoeff, MPI_DOUBLE, jProcessor, 0, MPI_COMM_WORLD, nullptr);
        }
